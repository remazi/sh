...
def masg(call):
	...
	if call.data =="SidraTools":
		...
		A = types.InlineKeyboardButton(text ="USER (#****#)", callback_data="F1")

		E = types.InlineKeyboardButton(text ="USER (#_#_#)", callback_data="F2")
		
		M = types.InlineKeyboardButton('المطور', url='https://t.me/G_D_I')
		...
		xn = "1234567890QAZWSXEDCRFVTGBYHNUJMIKLOP"
		xa = "_"
		xm = "1234567890QAZWSXEDCRFVTGBYHNUJMIKLOP"
		ok=0
		cp=0
		sk=0
		while True:
			...
			username = str(us)+str(ua)+str(un)+str(ua)+str(bs)
			url = "https://t.me/"+str(username)
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
...
if __name__ == "__main__":
    bot.remove_webhook()
    bot.set_webhook(url="https://itsemario.herokuapp.com/"+str(BOT_TOKEN))
